...
swagErr_t SwagInit(swag_t* refSwag, size_t cpcty) {
    ...
    refSwag -> data = (swagType*)calloc(cpcty + 2, sizeof(swagType));
    refSwag -> size = 0;
...
swagErr_t SwagPush(swag_t* refSwag, swagType val) {
    ...
    if (refSwag -> size == refSwag -> capacity) {
        refSwag -> capacity *= 2;
        refSwag -> data = (swagType*)realloc(refSwag -> data, 2*(refSwag -> capacity) + 2);
